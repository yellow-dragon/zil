...
while true; do
  ...
  if echo "$response" | grep -q "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="; then
    ...
    if [ "$qubic" -eq 0 ]; then
      killall aleominer 
      screen -S qubic -dm bash -c "/app-data/miners/apoolminer-2.6.2/apoolminer --algo qubic --account CP_e2sig0aa15 --worker $(echo $(hostname) | awk '{print substr($0, 1, 15)}') --pool 8.217.123.224:3334 --rest --cpu-off"      
      qubic=1
      aleo=0
    fi
  fi
